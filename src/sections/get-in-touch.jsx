    ...
    return (
        <section className="flex flex-col items-center" id="contact">
            ...
            <form onSubmit={handleSubmit} className='grid sm:grid-cols-2 gap-3 sm:gap-5 max-w-3xl mx-auto text-slate-400 mt-16 w-full' >
                ...
                >
                    <label className='font-medium text-slate-200'>Email id ✉️</label>
                ...
                >
                    ...
                    <input name='phone' type="phone" placeholder='Enter your Mobile Number with country code' className='w-full mt-2 p-3 outline-none border border-slate-700 rounded-lg focus-within:ring-1 transition focus:ring-indigo-600' />
                </motion.div>
...
const handleSubmit = (e) => {
  ...
  const phoneUser = formData.get("phone");
  const messageUser = formData.get("message");

  const phone = "918302442928"; // Your WhatsApp number (without +)
